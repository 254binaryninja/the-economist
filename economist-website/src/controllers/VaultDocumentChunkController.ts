...
} from "../domain/repository/IVaultDocumentChunkRepository";

@injectable()
export class VaultDocumentChunkController {
  constructor(
    @inject(TYPES.IVaultDocumentChunkRepository)
    private vaultDocumentChunkRepository: IVaultDocumentChunkRepository,
  ) {}

  async getByVaultDocumentId(vaultDocumentId: string, token: string) {
    await this.vaultDocumentChunkRepository.setToken(token);
    return this.vaultDocumentChunkRepository.getByVaultDocumentId(
      vaultDocumentId,
    );
  }
