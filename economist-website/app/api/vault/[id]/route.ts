...
export async function POST(
  ...
  try {
    ...
    if (!messages || !Array.isArray(messages)) {
      return new Response("Invalid payload: messages array is required", {
        status: 400,
      });
    }

    if (messages.length === 0) {
      return new Response("Invalid payload: messages array cannot be empty", {
        status: 400,
      });
    }
    ...
    const selectedSystem: SystemPromptKey =
      system && system in systemPrompts
        ? (system as SystemPromptKey)
        : "normal";
    ...
    const redisContextController = serverContainer.get<RedisContextController>(
    ...
    const vaultMessagesController =
      serverContainer.get<VaultMessagesController>(
    ...
    await vaultMessagesController.insertMessageToVault(
      ...
      token,
    ); // Embed last user message and do a similarity search in Pinecone
    ...
    console.log(
      `Using ${selectedSystem} economic perspective for vault ${vaultId}`,
    );
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("vault")) {
        return new Response("Vault not found or access denied", {
          status: 404,
        });
      }
    }
