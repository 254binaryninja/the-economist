...
export function useVaultDocuments(vaultID: string) {
  ...
  const vaultDocumentController = clientContainer.get<VaultDocumentController>(
  ...
  const fetchDocuments = async () => {
    ...
    try {
      if (isSignedIn && isLoaded) {
        ...
        const fetchDocuments = await vaultDocumentController.getByVaultId(
          vaultID,
          token,
        );
  ...
  const deleteDocument = async (documentId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/vault/${vaultID}/documents/${documentId}`,
        {
          method: "DELETE",
        },
      );
