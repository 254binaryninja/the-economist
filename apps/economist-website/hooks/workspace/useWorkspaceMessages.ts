...
export function useWorkspaceMessages(workspaceId: string) {
  ...
  const workspaceMessagesController =
    clientContainer.get<WorkspaceMessagesController>(
  ...
  const toggleUpvoting = async (
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === id ? { ...msg, is_upvoted: newUpvoteStatus } : msg,
        ),
      );
    ...
    try {
      if (isSignedIn && isLoaded) {
        ...
        const fetchedMessages =
          await workspaceMessagesController.getMessagesByWorkspaceId(
            workspaceId,
            token,
          );
