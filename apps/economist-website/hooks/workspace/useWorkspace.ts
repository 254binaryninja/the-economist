...
export function useWorkspace() {
  ...
  const workspaceController = clientContainer.get<WorkspaceController>(
  ...
  const fetchWorkspaces = async () => {
    ...
    try {
      if (isSignedIn && isLoaded) {
        ...
        const fetchedWorkspaces = await workspaceController.getByUserId(
          user_id,
          token,
        );
  ...
  const createWorkspace = async (
    ...
    try {
      ...
      const createdWorkspace = await workspaceController.create(
        { id, title, user_id },
        token,
      );
