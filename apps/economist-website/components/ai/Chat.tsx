    ...
    async (
      ...
      try {
        ...
        const attachments = options?.files
          ? await filesToAttachments(options.files)
          : [];
  ...
  return (
    <div className="flex flex-col h-full">
      <MessageList
        ...
        isLoading={isLoading}
        onToggleUpvote={(id: string, voteType: "upvote" | "downvote") =>
          toggleUpvoting(id, voteType)
        }
      />
      ...
      <div className="flex justify-center items-center mt-3 p-3">
        <p className="text-sm text-muted-foreground">
          AI can make mistakes. Please verify the information before making any
          decisions.
        </p>
      </div>
    </div>
