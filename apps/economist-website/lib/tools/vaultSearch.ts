...
export const vaultSearchTool = tool({
  description:
    "Search through documents in a vault using semantic similarity. Helps find relevant information from uploaded documents.",
  parameters: z.object({
    ...
    scoreThreshold: z
      ...
      .optional()
      .default(0.75)
      .describe("Minimum similarity score threshold (default: 0.75)"),
    documentType: z
      .string()
      .optional()
      .describe("Filter by document type (pdf, docx, txt, etc.)"),
  }),
  execute: async ({ vaultId, query, topK, scoreThreshold, documentType }) => {
    try {
      ...
      return createSuccessResponse({
        ...
        instruction:
          "This tool should be integrated with the vault search API endpoint to provide actual search results.",
      });
    } catch (error) {
...
export const vaultDocumentTool = tool({
  description:
    "Get information about documents in a vault, including metadata and summaries.",
  parameters: z.object({
    vaultId: z.string().describe("The ID of the vault to get documents from"),
    documentId: z
      .string()
      .optional()
      .describe("Specific document ID to get details for"),
  }),
    ...
    try {
      ...
      if (documentId) {
        return createSuccessResponse({
          ...
          instruction:
            "This tool should be integrated with GET /api/vault/[id]/documents/[documentId] endpoint.",
        });
      } else {
        return createSuccessResponse({
          ...
          instruction:
            "This tool should be integrated with GET /api/vault/[id]/documents endpoint.",
        });
      }
    } catch (error) {
