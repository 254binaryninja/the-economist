...
const economicNewsSchema = z.object({
  entity: z
    ...
    .optional()
    .describe(
      "Stock symbol or company name to filter news (e.g., AAPL, Tesla)",
    ),
  minSentiment: z
    .number()
    .min(-1)
    ...
    .optional()
    .describe(
      "Minimum sentiment score (-1 to 1, where -1 is very negative, 1 is very positive)",
    ),
  category: z
...
export const economicNewsTool = tool({
  description:
    "Fetch the latest economic and financial news articles from multiple sources (Marketaux and Finnhub). Returns comprehensive news data with sentiment analysis, perfect for market research and analysis.",
  parameters: economicNewsSchema,
  execute: async (input) => {
    try {
      const { entity, minSentiment, category } =
        economicNewsSchema.parse(input);
      const controller = clientContainer.get<EconomicNewsFeedController>(
        TYPES.EconomicsNewsFeedController,
      );

      const result = await controller.fetchNews({
        entity,
        minSentiment,
        category,
      });
      ...
      return {
        ...
        error: {
          ...
          type: "FETCH_ERROR",
          details:
            "Failed to fetch economic news from APIs. This could be due to network issues, API rate limits, or invalid parameters.",
          fallback:
