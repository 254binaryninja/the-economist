...
export async function fetchLastMessagesFromDB (workspaceId: string, limit: number,token:string): Promise<WorkspaceMessage[]> {
...
export async function saveMessageToDB (workspace_id: string, message: WorkspaceMessage, token: string): Promise<WorkspaceMessage | null> {
    const supabaseWithAuth = supabaseManager.getClient(token);
    const { data, error } = await supabaseWithAuth
        .from('workspace_messages')
