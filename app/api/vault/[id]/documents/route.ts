...
export async function POST(
  ...
  try {
    ...
    const vaultDocumentController =
      serverContainer.get<VaultDocumentController>(
        TYPES.VaultDocumentController,
      );
    const vaultDocumentChunkController =
      serverContainer.get<VaultDocumentChunkController>(
        TYPES.VaultDocumentChunkController,
      );
    const textEmbeddingController =
      serverContainer.get<TextEmbeddingController>(
        TYPES.TextEmbeddingController,
      );
    const pineconeVaultController =
      serverContainer.get<PineconeVaultController>(
        TYPES.PineconeVaultController,
      );
    ...
    if (file) {
      ...
      extractedText = await textEmbeddingController.extractTextFromUrl(url);
    } else {
      return Response.json(
        createErrorResponse(
    ...
    const documentMetadata =
      await textEmbeddingController.generateDocumentMetadata({
        text: extractedText,
    ...
    const textChunkEmbeddings =
      await textEmbeddingController.embedText(extractedText);
    ...
    const vectorIds = await pineconeVaultController.upsertChunks(
      vaultId,
      vectors,
    );
...
export async function GET(
  ...
  try {
    ...
    const documents = await vaultDocumentController.getByVaultId(
      vaultId,
      token,
    );

    return Response.json(createSuccessResponse({ documents }));
  } catch (error) {
