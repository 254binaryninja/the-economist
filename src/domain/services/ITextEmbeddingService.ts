...
import { DocumentMetadata, DocumentMetadataGenerator,ITextEmbeddingRepository, TextChunkEmbedding } from "../repository/ITextEmbeddingRepository";
...
export class ITextEmbeddingService implements ITextEmbeddingRepository {
    ...
    async extractText(filePath: string): Promise<string> {
        try{
         ...
         const text = await this.extractor.extractText({ input:buffer,type:'buffer' })
         if(!text) {
    ...
    async extractTextFromUrl(url: string): Promise<string> {
        try {
            const text = await this.extractor.extractText({input:url,type:'url'})
            if(!text){
    ...
    async generateDocumentMetadata(document: DocumentMetadataGenerator): Promise<DocumentMetadata> {
        try {
...
Please provide only the summary without any additional formatting or explanation.`;
            ...
            const documentName = firstLine.length > 0 
    ...
    async embedText(text: string): Promise<TextChunkEmbedding[]> {
        try {
            ...
            const textChunkEmbeddings: TextChunkEmbedding[] = chunks.map((chunk, index) => ({
                content: chunk,
