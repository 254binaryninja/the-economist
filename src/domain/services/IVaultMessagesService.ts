...
import { IVaultMessagesRepository, VaultMessage, VaultMessageInsert, VaultMessageUpdate } from "../repository/IVaultMessagesRepository";
...
export class VaultMessagesService implements IVaultMessagesRepository {
    ...
    async insertMessageToVault(vaultMessage: VaultMessageInsert): Promise<VaultMessage> {
       const supabaseWithAuth = this.getSupabaseClient();
       const { data,error } = await supabaseWithAuth
             .from('vault_messages')
             .insert(vaultMessage)
    ...
    async updateMessageInVault(id: string, vaultMessage: VaultMessageUpdate): Promise<VaultMessage | null> {
        const supabaseWithAuth = this.getSupabaseClient();
        const { data, error } = await supabaseWithAuth
              .from('vault_messages')
              .update(vaultMessage)
