...
import { IPineconeVaultRepository, QueryResult, VectorMetadata } from "../repository/IPineconeVaultRepository";
...
export class IPineconeVaultService implements IPineconeVaultRepository {
    ...
    async upsertChunks(namespace: string, chunks: Array<{ embedding: number[]; metadata: VectorMetadata; }>): Promise<string[]> {
        try {
    ...
    async query(namespace: string, embedding: number[], topK: number = 10, filter?: Record<string, any>): Promise<QueryResult[]> {
        try {
            const queryRequest = {
                vector: embedding,
                topK,
    ...
    async deleteByFilter(namespace: string, filter: Record<string, any>): Promise<void> {
        try {
