...
import {
  IVaultDocumentChunkRepository,
  VaultDocumentChunk,
  VaultDocumentChunkInsert,
  VaultDocumentChunkUpdate,
} from "../repository/IVaultDocumentChunkRepository";

@injectable()
export class IVaultDocumentChunkService
  implements IVaultDocumentChunkRepository
{
  ...
  async getByVaultDocumentId(
    vaultDocumentId: string,
  ): Promise<VaultDocumentChunk[]> {
    ...
    if (error) {
      console.error(
        "Error fetching document chunks by vault document ID:",
        error,
      );
      return [];
    }
  ...
  async create(
    vaultDocumentChunk: VaultDocumentChunkInsert,
  ): Promise<VaultDocumentChunk> {
    const supabaseWithAuth = this.getSupabaseClient();
    const { data, error } = await supabaseWithAuth
      .from("vault_document_chunks")
  ...
  async update(
    id: string,
    vaultDocumentChunk: VaultDocumentChunkUpdate,
  ): Promise<VaultDocumentChunk | null> {
    const supabaseWithAuth = this.getSupabaseClient();
    const { data, error } = await supabaseWithAuth
      .from("vault_document_chunks")
