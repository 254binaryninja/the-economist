...
import { IVaultWorkflowRepository, VaultWorkflow, VaultWorkflowInsert, VaultWorkflowUpdate } from "../repository/IVaultWorkflowRepository";
...
export class IVaultWorkflowService implements IVaultWorkflowRepository{
    ...
    async update(id: string, vaultWorkflow: VaultWorkflowUpdate): Promise<VaultWorkflow | null> {
        const supabaseWithAuth = this.getSupabaseClient();
        const { data, error } = await supabaseWithAuth
            .from('vault_workflows')
