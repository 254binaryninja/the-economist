...
import type { IVaultMessagesRepository, VaultMessageInsert, VaultMessageUpdate } from '../domain/repository/IVaultMessagesRepository';

@injectable()
export class VaultMessagesController {
    constructor(
        @inject(TYPES.IVaultMessagesRepository) private vaultMessagesRepository: IVaultMessagesRepository
    ) {}
    ...
    async updateMessageInVault(id: string, vaultMessage: VaultMessageUpdate, token: string) {
        await this.vaultMessagesRepository.setToken(token);
