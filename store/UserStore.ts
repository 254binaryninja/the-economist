...
import { User,UserInsert,UserUpdate } from "@/src/domain/repository/IUserRepository"


// Get controller from DI container
const userController = clientContainer.get<UserController>(TYPES.UserController);
...
export const useUserStore = create<UserStore>((set,get)=>({
    ...
    fetchUserData:async( token:string, user_id:string) => {
        ...
        try {
            ...
            set({ error: "Error fetching user data: " + (e as Error).message, isLoading: false });
        } finally {
            set({ isLoading:false})
        }
    },
    updateUserData:async(userData:UserUpdate,token:string)=> {
      ...
      try{
          ...
          set({ error: "Error updating user data: " + (e as Error).message, isLoading: false });
      } finally {
          set({isLoading:false})
      }
    }
